...
abstract class IdcPacket(val packetId: Int) {
    ...
    fun encode(sessionKey: ByteArray? = null): ByteBuffer {
        val body = encodeBody()
        val payload: ByteArray = if (sessionKey == null) {
            body
        } else {
    ...
    companion object {
        ...
        fun decode(frame: ByteBuffer, sessionKey: ByteArray? = null): IdcPacket? {
            val magic = frame.int
        ...
        fun create(id: Int): IdcPacket? = when (id) {
...
class RawIdcPacket(id: Int, var payload: ByteArray = ByteArray(0)) : IdcPacket(id) {
...
class LoginReq(
    ...
    private fun json() = """{"name":"${jsonEscape(name)}","app_ver_code":$appVerCode,"client_type":"${jsonEscape(clientType)}","dev_name":"${jsonEscape(devName)}","login_type":${loginType.ordinal},"login_magic_number":$loginMagicNumber,"encryption_algorithm_ver":$encryptionAlgorithmVer,"encryption_algorithm_detail":"${jsonEscape(encryptionAlgorithmDetail)}"}"""
...
class HeartBeat(var seq: Int = 0) : IdcPacket(IdcConst.ID_HEARTBEAT) {
    override fun encodeBody(): ByteArray {
        val b = ByteBuffer.allocate(4)
        b.putInt(seq)
        return b.array()
    }
    override fun decodeBody(buf: ByteBuffer) { seq = buf.int }
}
...
class VConnSyn(var moduleId: Int = 0) : IdcPacket(IdcConst.ID_VCONN_SYN) {
    override fun encodeBody(): ByteArray {
        val s = """{"mid":$moduleId}"""
    ...
    override fun decodeBody(buf: ByteBuffer) { moduleId = parseJsonObject(buf.getLPString()).int("mid") }
}

class VConnData(var moduleId: Int = 0, var payload: ByteArray = ByteArray(0)) : IdcPacket(IdcConst.ID_VCONN_DATA) {
    override fun encodeBody(): ByteArray {
        val s = """{"mid":$moduleId}"""
...
class VConnFin(var moduleId: Int = 0) : IdcPacket(IdcConst.ID_VCONN_FIN) {
    override fun encodeBody(): ByteArray {
        val s = """{"mid":$moduleId}"""
        val b = ByteBuffer.allocate(lpStringSize(s))
        b.putLPString(s)
...
class OpCmdKey(var keyCode: Int = 0, var keyOp: Int = 0) : IdcPacket(IdcConst.ID_OPCMD_KEY) {
    override fun encodeBody(): ByteArray {
...
class ImeTextChange(var text: String = "", var cursorPos: Int = 0) : IdcPacket(IdcConst.ID_IME_TEXT_CHANGE) {
    override fun encodeBody(): ByteArray {
...
class ImeAction(var actionId: Int = -1) : IdcPacket(IdcConst.ID_IME_ACTION) {
    override fun encodeBody(): ByteArray {
        val b = ByteBuffer.allocate(4)
        b.putInt(actionId)
        return b.array()
...
class ScreenShotResp(var imgData: ByteArray = ByteArray(0)) : IdcPacket(IdcConst.ID_CMD_SCREENSHOT_RESP) {
    override fun encodeBody() = ByteArray(0)
    override fun decodeBody(buf: ByteBuffer) {
        ...
        if (buf.remaining() >= 4) {
            ...
            val n = buf.int
            imgData = if (n > 0 && n == buf.remaining()) {
                val b = ByteArray(n); buf.get(b); b
            } else {
                buf.position(mark)
...
class DevNameUpdate(var devName: String = "") : IdcPacket(IdcConst.ID_DEVINFO_UPDATE_NAME) {
    override fun encodeBody() = ByteArray(0)
    override fun decodeBody(buf: ByteBuffer) {

...
- **Cmd packet framing**: CmdReqBase family (ScreenShot/SysProp/PackageInfo/PathInfo; 代码目前仅实现 ScreenShotReq/Resp) body = `LPString({"cmdReqID":N})` + `LPString({params})`; `Cmd_LaunchSth`(20400) is the exception — single LPString, sent raw without req/resp pairing
...
- None (self-contained; consumed by `app.tvlink.device`)

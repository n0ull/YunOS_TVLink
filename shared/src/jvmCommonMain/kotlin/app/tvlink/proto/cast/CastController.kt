...
class CastController(
    ...
    /** content-length 协议违规判定：header 在但值非法（非数字/负值/超 Int 范围）。header 缺省不算（按 0 容错）。 */
    private fun isInvalidContentLength(
        clHeader: String?,
        parsedLen: Long?,
    ): Boolean = clHeader != null && (parsedLen == null || parsedLen < 0L || parsedLen > Int.MAX_VALUE)
